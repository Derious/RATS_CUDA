...
int OverlappingTemplateMatchings(double alpha, unsigned char *data, int bits, int m)
{
	...
	if ( (sequence = (unsigned char *) calloc(m, sizeof(unsigned char))) == NULL ) {
        return 0;//memroy alloc failure
	...
	for ( i=0; i<N; i++ ) {
		...
		for ( j=0; j<M-m+1; j++ ) {
			match = 1;
			for ( k=0; k<m; k++ ) {
				if ( sequence[k] != GET_EPSILON(data,i*M+j+k) )
					match = 0;
			}
			if ( match == 1 )
				W_obs++;
	...
	if ( isNegative(p_value) || isGreaterThanOne(p_value) ){
        return 0;//WARNING:  P_VALUE IS OUT OF RANGE.
    }
